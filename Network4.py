...
class Generator(object):
    ...
    def generator(self):
	    ...
	    model = BatchNormalization(momentum = 0.5)(model)
	    model = add([gen_model, model], name="add_17_all")
...
class ResidueAdd(object):
    ...
    def residueadd(self):
        ...
        model = Conv2D(filters = 64, kernel_size = 3, strides = 1, padding = "same", name="conv2d_46_all")(dis_input)
        model = LeakyReLU(alpha = 0.2)(model)
        ...
        model = Conv2D(filters = 64, kernel_size = 3, strides = 1, padding = "same")(model)
        model = LeakyReLU(alpha = 0.2, name="leaky_re_lu_22_all")(model)
        model = UpSampling2D((2, 2))(model)   
