...
class ResidueAdd(object):
    ...
    def residueadd(self):
        ...
        # model = BinaryConvolution2D(64, 3, 3, input_shape=(self.image_shape[2], self.image_shape[0], self.image_shape[1]),
        #                       H=H, W_lr_multiplier=W_lr_multiplier, 
        #                       border_mode='same', bias=bias, name='conv1')(dis_input)
        model = Conv2D(filters = 64, kernel_size = 3, strides = 1, padding = "same")(dis_input)
        model = LeakyReLU(alpha = 0.2)(model)
        model = UpSampling2D((2, 2))(model)   
        # model = BinaryConvolution2D(3, 3, 3, input_shape=(64, 96, 96),
        #                       H=H, W_lr_multiplier=W_lr_multiplier, 
        #                       border_mode='same', bias=bias, name='conv1')(dis_input)
        model = Conv2D(filters = 3, kernel_size = 3, strides = 1, padding = "same")(model)
        model = LeakyReLU(alpha = 0.2)(model)
